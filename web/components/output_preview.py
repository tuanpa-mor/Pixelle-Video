...
from web.utils.cost_estimator import estimate_video_cost
        ...
        if st.button(tr("btn.generate"), type="primary", use_container_width=True):
            ...
            try:
                ...
                if st.session_state.get("debug_mode", False):
                    render_cost_estimate(result, video_params)
                    st.markdown("---")
...
def render_cost_estimate(result, video_params):
    """Render best-effort cost estimate when debug mode is enabled."""
    storyboard = result.storyboard
    config = storyboard.config
    frame_template = config.frame_template or ""

    if "/image_" in frame_template or frame_template.endswith("image_default.html") or "image_" in frame_template:
        template_type = "image"
    elif "/video_" in frame_template or "video_" in frame_template:
        template_type = "video"
    else:
    ...
    estimate = estimate_video_cost(
        ...
        text_length=len(video_params.get("text", "") or ""),
        mode=video_params.get("mode", "generate"),
        frame_count=len(storyboard.frames),
        duration_seconds=float(result.duration or 0),
        tts_mode=video_params.get("tts_inference_mode", "local"),
    ...
    st.markdown(f"**{tr('debug.cost_title')}**")
    st.caption(tr("debug.cost_caption"))
    total_col, duration_col, frames_col = st.columns(3)
    total_col.metric(tr("debug.cost_total"), f"${estimate.total_usd:.4f}")
    duration_col.metric(tr("debug.cost_duration"), f"{result.duration:.1f}s")
    frames_col.metric(tr("debug.cost_frames"), len(storyboard.frames))

    for item in estimate.breakdown:
        st.info(f"{item['name']}: ${item['usd']:.4f} — {item['detail']}")

    with st.expander(tr("debug.cost_assumptions"), expanded=False):
